...
schedule = schedule.asfreq('min').interpolate()  # Convert TimeSeries to specified frequency.
...
def sun(weather_df):
    ...
    weather_df['b_r'] = np.deg2rad((weather_df.hour / 24 - 81) * 360 / 365)  # (n - 81) / 365 * 360 n -- day of year
    weather_df['e'] = (9.87 * np.sin(2 * weather_df.b_r) - 7.53 * np.cos(weather_df.b_r) - 1.5 * np.sin(
        weather_df.b_r)) / 60
    weather_df['tas'] = np.mod(weather_df.hour, 24) + weather_df.e + (
            project['longitude'] - 15 * project['time_zone']) / 15
...
class Face(object):
    ...
    def __init__(self, orientation, tilt):
        ...
        self.Fg = 1 - self.Fs
        self.I_D = np.multiply(weather_df['Direct-Solar-Radiation'], self.cos_theta)  # 直達日射
        self.I_s = np.multiply(weather_df['Diffused-Solar-Radiation'], self.Fs)  # 天空日射部分
        self.I_hol = (np.multiply(weather_df['Direct-Solar-Radiation'], weather_df.sin_h)
                      + weather_df['Diffused-Solar-Radiation'])  # 水平面全天日射量
        self.I_r = np.multiply(self.I_hol, project['rho_g'] * self.Fg)  # rho_g地面反射率
        self.I_w = self.I_D + self.I_s + self.I_r  # 全日射量
...
class Windows(Face):
    ...
    def __init__(self, window_df):
        ...
        self.tau = float(window_df.window_tau)  # 透過率
        self.bn = float(window_df.window_BN)  # 吸収日射取得率
        self.k = float(window_df.window_K)  # 熱貫流率
        super().__init__(window_df.orientation, window_df.tilt)

        self.alpha_0 = project['alpha_i']  # 室内伝達係数
        self.alpha_m = project['alpha_o']  # 室外伝達係数

        self.FI = (1 - self.k / self.alpha_0)  # 吸収日射流入の割合
        ...
        self.CI_D = (3.4167 * self.cos_theta - 4.3890 * self.cos_theta ** 2 + 2.4948 *
                     self.cos_theta ** 3 - 0.5224 * self.cos_theta ** 4)  # 入射透過率と垂直入射透過率の比率
        self.GT = (self.glass_area * self.tau * (self.CI_D * self.I_D + 0.91 * (self.I_r + self.I_s))).values  # 透過熱
        self.GA = self.glass_area * self.bn * (self.CI_D * self.I_D + 0.91 * (self.I_r + self.I_s))  # 吸収熱
...
class Walls(Face):
    ...
    def __init__(self, wall_df):
        ...
        self.ur = [project['dt'] * 2 / (self.cap[i] + self.cap[i + 1]) / self.r[i + 1] for i in
                   range(sum(self.grid) + 1)]
...
class Rooms(object):
    def __init__(self, room_df):
        ...
        self.n_air = float(room_df.n_air)  # air exchange
        self.HVAC_schedule = {0: schedule[0], 1: schedule[1]}[room_df.HVAC_schedule].values  # select patton of two
        self.human_n = float(room_df.human_n)  # number
        self.human_t = float(room_df.human_t)  # sensible + latent
        self.human_s24 = float(room_df.human_s24)  # sensible when 24
        self.human_d = float(room_df.human_d)  # parameter
        ...
        self.equipment_wl = float(room_df.equipment_wl)  # latent?
        ...
        self.Q_HS = 0
        self.Q_HL = 0
        self.human_kc = 0.5  # convection in sensible
        self.human_kr = 1 - self.human_kc  # radiation in sensible
        ...
        # 房间固有属性
        self.Arm = sum([x.area for x in self.envelope])  # 室内全表面積
        self.ANF = sum([x.anf for x in self.envelope])
        self.SDT = self.Arm - project['kr'] * self.ANF  # kr rate of radiation
    ...
    def BR(self, step):
        ...
        self.BRC = (self.RMDT * self.indoor_temp + self.CA + project['c_air'] * self.Go * outdoor_temp[
            step] + self.HG_c)
...
class DuctSystem(object):
    ...
    def __init__(self, duct_supply_air, duct_return_air, duct_exhaust_air, duct_fresh_air, duct_mix_air, fan_s, fan_r,
                 dp_ahu=200):
    ...
    def air_state_cal(self, ex):
        ...
        # mixed air state
        self.mixed_air_temp = (self.indoor_temp_avg * self.indoor_air_g + self.fresh_air_temp * self.fresh_air_g) / (
                self.indoor_air_g + self.fresh_air_g)
        self.mixed_air_humidity = (
                                          self.indoor_humidity_avg * self.indoor_air_g + self.fresh_air_humidity * self.fresh_air_g) / (
                                          self.indoor_air_g + self.fresh_air_g)
...
def t2pws(t):
    t = t + 273.15
    return (np.exp(
        -5800 / t + 1.391 - 0.04864 * t + 0.4176e-4 * np.power(t, 2) - 0.1445e-7 * np.power(t, 3) + 6.546 * np.log(
            t))) / 1000
...
class HeatExchanger(object):
    ...
    def epsilon_to_temp(self):
        ...
        if cgh < cgc:
            ...
            self.q = self.flow_water * self.rho_water * self.c_water * (
                    - self.t_water_in + self.t_water_out) / 3600 * 1000
...
def deltatemp2flow(room, season):
    ...
    if deltatemp < -1:
        ...
        room.g_set = room.gmin + (deltatemp + 1) * (room.gmax - room.gmin) / 3 * 2
...
def room_control(room, step, season, method='flow'):
    ...
    if room.mode:
        ...
        room.duct.damper.theta, room.e, room.es = pid_control(target, set_point, control0, p, i, d, room.e, room.es,
                                                              control_max=70, control_min=0, tf=tf)
...
def duct_system_control(system, method='flow', co2_method=True, supply_air_temp_reset=True):
    ...
    if system.mode:
        ...
        system.fan_s.inv, system.fan_s.e, system.fan_s.es = pid_control(target_s, set_point_s, control0_s, p_s, i_s,
                                                                        d_s, system.fan_s.e, system.fan_s.es,
                                                                        control_max=50, control_min=15, tf=tf)

        # fan_r control by pid
        system.fan_r.inv, system.fan_r.e, system.fan_r.es = pid_control(target_r, set_point_r, control0_r, p_r, i_r,
                                                                        d_r, system.fan_r.e, system.fan_r.es,
                                                                        control_max=50, control_min=15, tf=tf)
        ...
        if co2_method:
            ...
            i = 0
            d = 0
            system.duct_mix_air.damper.theta, system.duct_mix_air.damper.e, system.duct_mix_air.es = pid_control(target,
                                                                                                                 set_point,
                                                                                                                 control0,
                                                                                                                 p, i,
                                                                                                                 d,
                                                                                                                 system.duct_mix_air.damper.e,
                                                                                                                 system.duct_mix_air.damper.es,
                                                                                                                 control_max=70,
                                                                                                                 control_min=0,
                                                                                                                 tf=tf)
            # F8
        ...
        i = 0
        d = 0
        system.water_flow, system.water_flow_e, system.water_flow_es = pid_control(target, system.supply_air_set_point,
                                                                                   control0, p, i, d,
                                                                                   system.water_flow_e,
                                                                                   system.water_flow_es, control_max=10,
                                                                                   control_min=0, tf=tf)
...
for cal_step in range(int((end - start).view('int64') / project['dt'] / 10e8)):
    ...
    dataset.extend([duct_system.set_point_pressure, rooms[0].duct.p, f2.inv, f1.inv, duct_system.supply_air_t,
                    duct_system.mixed_air_temp, duct_system.duct_mix_air.damper.theta])
    dataset.extend([duct_system.supply_air_set_point, HE_ahu.t_water_out, duct_system.water_flow, f2.p / 10,
                    outdoor_temp[cal_step], outdoor_RH, energy / 1000])
